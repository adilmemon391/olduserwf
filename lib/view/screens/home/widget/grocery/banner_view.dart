      ...
      return (bannerList != null && bannerList.isEmpty) ? const SizedBox() : Container(
        ...
        child: bannerList != null ? Column(
          ...
          children: [
            ...
            //       return InkWell(
            //         onTap: () async {
            //           var bannerData = bannerDataList[index];
            //           if (bannerData is Item) {
            //             Get.find<ItemController>().navigateToItemPage(bannerData, context);
            //           } else if (bannerData is Store) {
            //             Store? store = bannerData;
            //             if (isFeatured && Get.find<SplashController>().moduleList != null) {
            ...
            //               arguments:StoreScreen(store: store, fromModule: isFeatured),
            //             );
            //           } else if (bannerData is BasicCampaignModel) {
            //             BasicCampaignModel campaign = bannerData;
            //             Get.toNamed(RouteHelper.getBasicCampaignRoute(campaign));
            //           } else if (bannerData is String) {
            //             String url = bannerData;
            //             if (await canLaunchUrlString(url)) {
            //               await launchUrlString(url, mode:LaunchMode.externalApplication);
            //             } else {
            //               showCustomSnackBar('unable_to_found_url'.tr);
