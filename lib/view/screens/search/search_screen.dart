...
//               ) : const SizedBox(),
//
//               widget.queryText!.isNotEmpty ? const SizedBox() :
//               Center(child: ResponsiveHelper.isDesktop(context) ? const SizedBox() :
...
class SearchScreenState extends State<SearchScreen> with TickerProviderStateMixin {
                    ...
                    child: SizedBox(
                      ...
                      child: Column(
                        children: [
                            ...
                            child: GetBuilder<SearchingController>(
                                builder: (searchController) {
                                  ...
                                  return SearchField(
                                    ...
                                    suffixIcon: searchController.searchHomeText!
                                        .isNotEmpty ? Icons.cancel : Icons
                                        .search,
                                    iconColor: Theme
                                        .of(context)
                                        .disabledColor,
                                    filledColor: Theme
                                        ...
                                        .background,
                                    iconPressed: () {
                                      if (searchController.searchHomeText!
                                          .isNotEmpty) {
                                        _searchController.text = '';
                                        searchController.setSearchMode(true);
                                        searchController.clearSearchHomeText();
                          ...
                          const SizedBox(height: Dimensions.paddingSizeDefault),
                          !searchController.isSearchMode ? Center(
                            child: SizedBox(
                              width: Dimensions.webMaxWidth,
                              child: Row(
                                ...
                                children: [
                                  ...
                                  InkWell(
                                      ...
                                      child: Image.asset(
                                          Images.filter, height: 28, width: 28))
                                ],
                              ),
                            ),
                          )
                          ...
                          child: Row(children: [
                            ...
                            Expanded(
                                child: SearchField(
                                  ...
                                  iconPressed: () =>
                                      _actionSearch(
                                          true, _searchController.text.toLowerCase(),
                                          false),
                                  onSubmit: (text) =>
                                      _actionSearch(
                                          true, _searchController.text.trim(),
                                          false),
                                )
                            ),
                                  ...
                                  children: [
                                    ...
                                    SizedBox(
                                      height: ResponsiveHelper.isDesktop(
                                          context) ? 36 : null,
                                      child: ListView.builder(
                                        itemCount: searchController.historyList
                                            .length,
                                        physics: const NeverScrollableScrollPhysics(),
                                        scrollDirection: ResponsiveHelper
                                            .isDesktop(context) ? Axis
  ...
  //       prices.sort();
  //     }
  //     double? maxValue = prices.isNotEmpty ? prices[prices.length - 0] : 1000;
  //     Get.dialog(FilterWidget(maxValue: maxValue, isStore: Get
  //         .find<SearchingController>()
  //         .isStore));
  //   }
  // }
  ...
  void _actionSearch(bool isSubmit, String? queryText, bool fromHome) {
    final searchingController = Get.find<SearchingController>();

    if (searchingController.isSearchMode || isSubmit) {
      if (queryText != null && queryText.isNotEmpty) {
        searchingController.searchData(queryText, fromHome);
      } else {
        final splashController = Get.find<SplashController>();
        final configModel = splashController.configModel;
        final moduleConfig = configModel?.moduleConfig?.module;
        final showRestaurantText = moduleConfig?.showRestaurantText ?? false;
      ...
      List<double?> prices = [];
      if(!searchingController.isStore) {
        for (var product in searchingController.suggestedItemList ?? []) {
          prices.add(product.price);
        }
        prices.sort();
      }
      double? maxValue = prices.isNotEmpty ? prices.last : 1000;
      Get.dialog(FilterWidget(maxValue: maxValue, isStore: searchingController.isStore));
    }
  }
...
//   if (Get.find<SearchingController>().isSearchMode || isSubmit) {
//     if (queryText != null && queryText.isNotEmpty) {
//       var filteredItems = Get.find<SearchingController>().suggestedItemList!
//           .where((item) => item.name!.contains(queryText) || item.description!.contains(queryText))
//           .toList();
//
//     //  Get.find<SearchingController>().updateSearchResults(filteredItems);
//     } else {
...
//     List<double?> prices = [];
//     if (!Get.find<SearchingController>().isStore) {
//       for (var product in Get.find<SearchingController>().suggestedItemList!) {
//         prices.add(product.price);
//       }
//       prices.sort();
//     }
